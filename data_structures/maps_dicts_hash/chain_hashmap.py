...
class ChainHashMap[K, V](HashMapBase):
    ...
    def _bucket_getitem(self, j: int, k: K) -> V:
        ...
        Args:
            ...
            k (K): clave de la entrada que se busca.

        Raises:
            KeyError: Arroja KeyError si el bucket indicado por j no tiene elementos.

        Returns:
            V: devuelve el valor correspondiente a la clave k que está en el bucket de la posición j.
        """
    ...
    def _bucket_setitem(self, j : int , k : K, v : V):
        """ Inserta/actualiza con v como nuevo valor la entrada con clave k
        en el bucket j.

        Args:
            j (int): posición que resultó de la función hash sobre k.
            k (K): clave de la entrada que corresponde insertar/actualizar.
            v (V): nuevo valor de la entrada.
        """
    ...
    def _bucket_delitem(self, j : int, k : K):
        """ Elimina del bucket en la posición j la entrada con clave k.

        Args:
            j (int): posición que resultó de la función hash sobre k.
            k (K): clave de la entrada a eliminar.
